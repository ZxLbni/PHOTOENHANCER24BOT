...
async def song(client, message):
    ...
    if 'data' not in r or 'results' not in r['data'] or len(r['data']['results']) == 0:
        await pak.edit("No results found for the given query.")
        return
    
    song_data = r['data']['results'][0]
    
    try:
        sname = song_data['name']
        slink = song_data['downloadUrl'][4]['link']
        ssingers = song_data['primaryArtists']
        img = song_data['image'][2]['link']
    except KeyError as e:
        await pak.edit(f"Error parsing song data: missing key {str(e)}")
        return
    ...
    try:
        ...
        await pak.edit(f"Error downloading media: {str(e)}")
        return
    ...
    await message.reply_audio(audio=ffile, title=sname, performer=ssingers, caption=f"[{sname}]({song_data['url']}) - from saavn", thumb=thumbnail)
